...
export default function ArticleCard({ article }: ArticlesCardProps) {
    return (
        <div key={article.id} className="overflow-hidden rounded-lg shadow-sm transition hover:shadow-lg ">
            ...
            <div className="p-4 sm:p-6">
                <Link href={`/blog/${article.id}`} className="font-medium hover:underline">
                    {article.title}
                </Link>
                <div className="flex flex-col space-y-2 lg:flex-row items-center justify-between mt-2">
                    ...
                    <div className="flex h-full items-center">
                        ...
                        <p className="flex flex-col text-xs text-gray-400"> {article.author?.name || "Desconhecido"}
                            <span className="text-xs text-gray-400">{formatDistanceToNow(new Date(article.createdAt ?? 'Data indisponivel'), { addSuffix: true, locale: ptBR })}</span>
                        </p>
                    </div>
