    ...
    <CardContainer>
      {services.length > 0 ? (
        services.map((service) => (
          <div
            key={service.id}
            className="flex flex-col h-full bg-white rounded-xl shadow-card relative z-10 p-7  px-9 text-left
      ...
      before:transition-all before:duration-1000 hover:before:w-full"
          >
            ...
            <h3 className="font-bold mb-0 transition-all duration-400 ease-out leading-[1.4]">
              {service.name}
            </h3>
            <p className="flex flex-grow text-[15px] mt-[15px] mb-0 transition-all duration-400 ease-out will-change-transform">
              {service.description}
            </p>
