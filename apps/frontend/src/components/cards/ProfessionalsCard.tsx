...
export default async function ProfessionalsCard() {
  ...
  return (
  <CardContainer gridClassName=" xl:grid-cols-4">
      {doctors.length > 0 ? (
    doctors.map((professional) => (
    <div
      key={professional.id}
      className='relative flex flex-col h-[540px] md:max-h-[440px] rounded-xl mt-[30px] overflow-hidden'>
            <div className='relative flex flex-grow  w-full group z-10 rounded-[5px] overflow-hidden mx-auto'>
              <Image
        src={
        ...
        />
            ...
            <div className='flex flex-col p-4 text-center bg-zinc-50  rounded-sm'>
              <h3 className='text-zinc-800 text-base font-bold mb-[5px]'>
                {professional.name}
              </h3>
              <p className='text-blue-600 text-[15px]'>
                {professional.specialties
          ? professional?.specialties[0]?.name: "Especialidade não encontrada"}
        </p>
      </div>
    </div>
    ))
