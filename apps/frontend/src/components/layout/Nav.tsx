      ...
      <div className="flex flex-col -mx-6 md:flex-row md:items-center md:mx-8">
       {navLinks.map((link) => (
        ...
        <Link
         key={link.name}
         href={link.href}
         className={`px-3 py-2 mx-3 md:mx-0 my-1 transition-colors duration-300 tranform rounded-md md:mt-0 hover:bg-gray-100 h-full ${pathname === link.href ? " md:text-brand-primary" : " text-gray-700"}`}
          ...
          <div className="flex flex-col">
           ...
           {link.dropDown.map((subLink) => (
            <Link
             key={subLink.name}
             href={subLink.href}
             className={`ms-2 px-3 py-2 mx-3 md:mx-0 my-1 transition-colors duration-300 tranform rounded-md md:mt-0 hover:bg-gray-100 h-full ${pathname === subLink.href ? " md:text-brand-primary" : " text-gray-700"}`}
            >
             {subLink.name}
            </Link>
           ))}
