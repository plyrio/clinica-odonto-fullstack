...
export class EmployeeController {
  ...
  @ApiResponse({ status: 503, description: 'Service unavailable.' })
  findAllDoctors() {
    return this.employeeService.findAllDoctors();
  }
  ...
  @ApiOperation({
    ...
    description:
      'Fetch the details of a specific doctor by their unique ID. The ID must correspond to an existing doctor in the database.',
  })
  @ApiParam({
    name: 'id',
  ...
  findOneDoctor(@Param('id') id: string) {
    return this.employeeService.findOneDoctor(+id);
  }
